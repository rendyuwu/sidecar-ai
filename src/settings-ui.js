export class SettingsUI {
  ...
  renderAddonsList() {
    ...
    if (!listContainer) {
      ...
      console.log(
        "[Sidecar AI] Add-ons list container not found (settings tab might be closed)"
      );
      return;
    }
    ...
    if (addons.length === 0) {
      listContainer.innerHTML =
        '<div class="add_ons_empty"><p>No Sidecars configured. Click "Create Sidecar" to create one.</p></div>';
    ...
    const allSelected =
      addons.length > 0 && addons.every((a) => this.selectedAddons.has(a.id));
    header.innerHTML = `
            <div class="add_ons_list_header_content">
                <label class="add_ons_checkbox_label">
                    <input type="checkbox" id="add_ons_select_all" ${
                      allSelected ? "checked" : ""
                    } class="add_ons_select_all_checkbox">
                    <span>Select All</span>
                </label>
                <span class="add_ons_selected_count" id="add_ons_selected_count">${
                  this.selectedAddons.size
                } selected</span>
            </div>
        `;
      ...
      item.innerHTML = `
                <div class="add_ons_item_header">
                    <div class="add_ons_item_select">
                        <label class="add_ons_checkbox_label">
                            <input type="checkbox" class="add_ons_item_checkbox" data-addon-id="${
                              addon.id
                            }" ${isSelected ? "checked" : ""}>
                            <span></span>
                        </label>
                    ...
                    <div class="add_ons_item_info">
                        ...
                        <span class="add_ons_item_meta">
                            ${enabledBadge}
                            <span class="add_ons_badge add_ons_badge_${triggerBadgeClass}">${
        addon.triggerMode || "auto"
      }</span>
                            <span class="add_ons_badge">${
                              addon.requestMode || "standalone"
                            }</span>
                            <span class="add_ons_badge">${
                              addon.responseLocation || "outsideChatlog"
                            }</span>
                            ${
                              addon.formatStyle &&
                              addon.formatStyle !== "html-css"
                                ? `<span class="add_ons_badge" title="Format Style">${addon.formatStyle}</span>`
                                : ""
                            }
                        </span>
                    </div>
        ...
        addon.id
      }" class="add_ons_enable_toggle">
                            <span class="add_ons_toggle_slider"></span>
                        </label>
                        <button class="menu_button add_ons_button_small" data-action="edit" data-addon-id="${
                          addon.id
                        }" title="Edit">
                            <i class="fa-solid fa-edit"></i>
                        </button>
                        <button class="menu_button add_ons_button_small" data-action="history" data-addon-id="${
                          addon.id
                        }" title="View History">
                            <i class="fa-solid fa-history"></i>
                        </button>
                        <button class="menu_button add_ons_button_small" data-action="duplicate" data-addon-id="${
                          addon.id
                        }" title="Duplicate">
                            <i class="fa-solid fa-copy"></i>
                        </button>
                        <button class="menu_button add_ons_button_small add_ons_button_danger" data-action="delete" data-addon-id="${
                          addon.id
                        }" title="Delete">
                ...
                ${
                  addon.description
                    ? `<p class="add_ons_item_description">${addon.description}</p>`
                    : ""
                }
            `;
  ...
  bindEvents() {
    ...
    $(document)
      .off("click.sidecar", "#sidecar_create_button")
      .on("click.sidecar", "#sidecar_create_button", function (e) {
        e.preventDefault();
        e.stopPropagation();
        self.openModal();
      });
    ...
    $(document)
      .off("click.sidecar", "#sidecar_ai_maker_button")
      .on("click.sidecar", "#sidecar_ai_maker_button", function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    // Templates button
    $(document)
      .off("click.sidecar", "#sidecar_templates_button")
      .on("click.sidecar", "#sidecar_templates_button", function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    // Export button
    $(document)
      .off("click.sidecar", "#sidecar_export_button")
      .on("click.sidecar", "#sidecar_export_button", function (e) {
        e.preventDefault();
        e.stopPropagation();
        self.handleExport();
      });
    ...
    // Import button
    $(document)
      .off("click.sidecar", "#sidecar_import_button")
      .on("click.sidecar", "#sidecar_import_button", function (e) {
        e.preventDefault();
        e.stopPropagation();
        self.handleImport();
      });
    ...
    // Modal close
    $(document)
      .off("click.sidecar", "#add_ons_modal_close, #add_ons_form_cancel")
      .on(
        "click.sidecar",
        "#add_ons_modal_close, #add_ons_form_cancel",
        function (e) {
          e.preventDefault();
          e.stopPropagation();
          self.closeModal();
    ...
    // Edit button
    $(document)
      .off("click.sidecar", '[data-action="edit"]')
      .on("click.sidecar", '[data-action="edit"]', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    // Delete button
    $(document)
      .off("click.sidecar", '[data-action="delete"]')
      .on("click.sidecar", '[data-action="delete"]', function (e) {
        e.preventDefault();
        e.stopPropagation();
        const addonId = $(this).data("addon-id");
    ...
    // Enable toggle
    $(document)
      .off("change.sidecar", ".add_ons_enable_toggle")
      .on("change.sidecar", ".add_ons_enable_toggle", function (e) {
        e.stopPropagation();
        const addonId = $(this).data("addon-id");
    ...
    // Save form
    $(document)
      .off("click.sidecar", "#add_ons_form_save")
      .on("click.sidecar", "#add_ons_form_save", function (e) {
        e.preventDefault();
        e.stopPropagation();
        self.saveAddon();
      });
    ...
    // Result format hint
    $(document)
      .off("change.sidecar", "#add_ons_form_result_format")
      .on("change.sidecar", "#add_ons_form_result_format", function (e) {
        e.stopPropagation();
        ...
        switch (format) {
          case "collapsible":
            hint.html(
              "Collapsible: &lt;details&gt; tag - <strong>Click to expand/collapse</strong> (best for long content)"
            );
            break;
          case "separate":
            hint.html(
              "Separate Block: <strong>Always visible</strong> with --- Name --- borders (good for short content)"
            );
            break;
          case "append":
            hint.html(
              "Append: <strong>Seamlessly inline</strong> - No wrapper, flows with message (use carefully)"
            );
            break;
        }
      });
    ...
    $(document)
      .off("change.sidecar", "#add_ons_form_response_location")
      .on("change.sidecar", "#add_ons_form_response_location", function (e) {
        e.stopPropagation();
        const location = $(this).val();
        const hint = $("#add_ons_response_location_hint");
        if (location === "chatHistory") {
          hint.text(
            "Chat History: Injected as HTML comment inside message (main AI can see it)"
          );
        } else {
          hint.text(
    ...
    $(document)
      .off("click.sidecar", ".add_ons_modal_content")
      .on("click.sidecar", ".add_ons_modal_content", function (e) {
        e.stopPropagation();
      });

    // Close modal when clicking backdrop (outside content)
    $(document)
      .off("click.sidecar", ".add_ons_modal")
      .on("click.sidecar", ".add_ons_modal", function (e) {
        if (e.target === this) {
          self.closeModal();
        }
      });
    ...
    $(document)
      .off("click.sidecar", "#add_ons_history_modal")
      .on("click.sidecar", "#add_ons_history_modal", function (e) {
        if (e.target === this) {
    ...
    $(document)
      .off("click.sidecar", "#add_ons_templates_modal")
      .on("click.sidecar", "#add_ons_templates_modal", function (e) {
        if (e.target === this) {
    ...
    $(document)
      .off("click.sidecar", "#add_ons_ai_maker_modal_close, #ai_maker_cancel")
      .on(
        "click.sidecar",
        "#add_ons_ai_maker_modal_close, #ai_maker_cancel",
        function (e) {
          e.preventDefault();
          e.stopPropagation();
    ...
    $(document)
      .off("click.sidecar", "#add_ons_ai_maker_modal")
      .on("click.sidecar", "#add_ons_ai_maker_modal", function (e) {
        if (e.target === this) {
          self.closeAIMakerModal();
        }
      });
    ...
    $(document)
      .off("click.sidecar", "#ai_maker_generate")
      .on("click.sidecar", "#ai_maker_generate", function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $(document)
      .off("click.sidecar", "#ai_maker_export")
      .on("click.sidecar", "#ai_maker_export", function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    // AI Maker add button
    $(document)
      .off("click.sidecar", "#ai_maker_add")
      .on("click.sidecar", "#ai_maker_add", function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $(document)
      .off("click.sidecar", "#ai_maker_test")
      .on("click.sidecar", "#ai_maker_test", function (e) {
        e.preventDefault();
        e.stopPropagation();
        self.testTemplate();
      });
    ...
    $(document)
      .off("click.sidecar", "#ai_maker_publish")
      .on("click.sidecar", "#ai_maker_publish", function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $(document)
      .off("click.sidecar", ".add_ons_modal select")
      .on("click.sidecar", ".add_ons_modal select", function (e) {
        e.stopPropagation();
      });

    // Format style change - ensure history depth minimum for beautify
    $(document)
      .off("change.sidecar", "#add_ons_form_format_style")
      .on("change.sidecar", "#add_ons_form_format_style", function (e) {
    ...
    $(document)
      .off("change.sidecar input.sidecar", "#add_ons_form_history_depth")
      .on(
        "change.sidecar input.sidecar",
        "#add_ons_form_history_depth",
        function (e) {
    ...
    $(document)
      .off("click.sidecar", "#add_ons_test_connection")
      .on("click.sidecar", "#add_ons_test_connection", function (e) {
        e.preventDefault();
        e.stopPropagation();
        self.testConnection();
      });
    ...
    $(document)
      .off("focus.sidecar input.sidecar", "#add_ons_form_api_key")
      .on("focus.sidecar input.sidecar", "#add_ons_form_api_key", function (e) {
        const $field = $(this);
    ...
    $(document)
      .off("change.sidecar", "#add_ons_form_include_history")
      .on("change.sidecar", "#add_ons_form_include_history", function (e) {
        e.stopPropagation();
    ...
    $(document)
      .off("change.sidecar", "#add_ons_form_trigger_mode")
      .on("change.sidecar", "#add_ons_form_trigger_mode", function (e) {
        e.stopPropagation();
        const mode = $(this).val();
    ...
    $(document)
      .off("change.sidecar", "#add_ons_form_trigger_type")
      .on("change.sidecar", "#add_ons_form_trigger_type", function (e) {
        e.stopPropagation();
    ...
    // Regex tester button
    $(document)
      .off("click.sidecar", "#add_ons_regex_test_btn")
      .on("click.sidecar", "#add_ons_regex_test_btn", function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $(document)
      .off("keypress.sidecar", "#add_ons_regex_test_input")
      .on("keypress.sidecar", "#add_ons_regex_test_input", function (e) {
        if (e.which === 13) {
          // Enter key
          e.preventDefault();
          self.testRegexPatterns();
    ...
    $(document)
      .off("change.sidecar", "#add_ons_select_all")
      .on("change.sidecar", "#add_ons_select_all", function (e) {
        e.stopPropagation();
        const checked = $(this).is(":checked");
    ...
    $(document)
      .off("change.sidecar", ".add_ons_item_checkbox")
      .on("change.sidecar", ".add_ons_item_checkbox", function (e) {
        e.stopPropagation();
        const addonId = $(this).data("addon-id");
        const checked = $(this).is(":checked");
        ...
        const allSelected =
          allAddons.length > 0 &&
          allAddons.every((a) => self.selectedAddons.has(a.id));
    ...
    // Bulk actions
    $(document)
      .off("click.sidecar", "#add_ons_bulk_enable")
      .on("click.sidecar", "#add_ons_bulk_enable", function (e) {
        e.preventDefault();
        e.stopPropagation();
        self.bulkEnable();
      });
    ...
    $(document)
      .off("click.sidecar", "#add_ons_bulk_disable")
      .on("click.sidecar", "#add_ons_bulk_disable", function (e) {
        e.preventDefault();
        e.stopPropagation();
        self.bulkDisable();
      });
    ...
    $(document)
      .off("click.sidecar", "#add_ons_bulk_delete")
      .on("click.sidecar", "#add_ons_bulk_delete", function (e) {
        e.preventDefault();
        e.stopPropagation();
        self.bulkDelete();
      });
    ...
    $(document)
      .off("click.sidecar", "#add_ons_bulk_duplicate")
      .on("click.sidecar", "#add_ons_bulk_duplicate", function (e) {
        e.preventDefault();
        e.stopPropagation();
        self.bulkDuplicate();
      });
    ...
    $(document)
      .off("click.sidecar", "#add_ons_bulk_clear")
      .on("click.sidecar", "#add_ons_bulk_clear", function (e) {
        e.preventDefault();
        e.stopPropagation();
        self.clearSelection();
      });
    ...
    // History button
    $(document)
      .off("click.sidecar", '[data-action="history"]')
      .on("click.sidecar", '[data-action="history"]', function (e) {
        e.preventDefault();
        e.stopPropagation();
        const addonId = $(this).data("addon-id");
    ...
    // Duplicate button
    $(document)
      .off("click.sidecar", '[data-action="duplicate"]')
      .on("click.sidecar", '[data-action="duplicate"]', function (e) {
        e.preventDefault();
        e.stopPropagation();
        const addonId = $(this).data("addon-id");
    ...
    $(document)
      .off(
        "click.sidecar",
        "#add_ons_templates_modal_close, #add_ons_templates_close"
      )
      .on(
        "click.sidecar",
        "#add_ons_templates_modal_close, #add_ons_templates_close",
    ...
    $(document)
      .off("click.sidecar", "#sidecar_browse_local_templates")
      .on("click.sidecar", "#sidecar_browse_local_templates", function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $(document)
      .off("click.sidecar", "#sidecar_browse_community_templates")
      .on("click.sidecar", "#sidecar_browse_community_templates", function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $(document)
      .off(
        "click.sidecar",
        "#add_ons_history_modal_close, #add_ons_history_close"
      )
      .on(
        "click.sidecar",
        "#add_ons_history_modal_close, #add_ons_history_close",
    ...
    // History filters
    $(document)
      .off("input.sidecar", "#add_ons_history_search")
      .on("input.sidecar", "#add_ons_history_search", function () {
        self.filterHistory();
      });

    $(document)
      .off("change.sidecar", "#add_ons_history_sort")
      .on("change.sidecar", "#add_ons_history_sort", function () {
        self.filterHistory();
      });

    $(document)
      .off("click.sidecar", "#add_ons_history_clear_filters")
      .on("click.sidecar", "#add_ons_history_clear_filters", function () {
  ...
  async loadModelsForProvider(provider, retryCount = 0, selectedModel = null) {
    ...
    if (models.length < minModelsExpected && retryCount < maxRetries) {
      ...
      console.log(
        `[Sidecar AI] ${provider} only found ${
          models.length
        } models (expected at least ${minModelsExpected}), retrying (${
          retryCount + 1
    ...
    if (
      provider === "openrouter" &&
      models.length < minModelsExpected &&
...
3. Network issues preventing model list fetch`);
  ...
  populateModelDropdown(modelSelect, models, selectedModel = null) {
    setTimeout(() => {
      ...
      models.forEach((model) => {
        ...
        if (
          selectedModel &&
          (model.value === selectedModel || model.id === selectedModel)
        ) {
  ...
  async getProviderModels(provider) {
    let models = [];
    console.log(
      "[Sidecar AI] ========== Loading models for provider:",
      provider,
      "=========="
    );
    ...
    if (provider === "openrouter") {
      console.log(
        "[Sidecar AI] Debug - window.openRouterModels:",
        typeof window.openRouterModels,
        window.openRouterModels?.length || "N/A"
      );
      console.log(
        "[Sidecar AI] Debug - window.model_list:",
        typeof window.model_list,
        window.model_list?.length || "N/A"
      );
      console.log(
        "[Sidecar AI] Debug - #model_openrouter_select exists:",
        $("#model_openrouter_select").length > 0
      );
      console.log(
        "[Sidecar AI] Debug - #model_openrouter_select option count:",
        $("#model_openrouter_select option").length
      );
      console.log(
        "[Sidecar AI] Debug - #openrouter_model exists:",
        $("#openrouter_model").length > 0
      );
      console.log(
        "[Sidecar AI] Debug - #openrouter_model option count:",
        $("#openrouter_model option").length
      );
    }
    ...
    for (const selector of domSelectors) {
      try {
        ...
        if ($el.length && $el.is("select")) {
          ...
          const hasSelect2 =
            $el.hasClass("select2-hidden-accessible") || $el.data("select2");
          ...
          if (hasSelect2) {
            try {
              ...
              if (select2Data && select2Data.length > 0) {
                ...
                console.log(
                  `[Sidecar AI] Found select2 dropdown at ${selector} with ${optionCount} options from select2 data`
                );
              }
            } catch (e) {
              console.log(
                `[Sidecar AI] select2 data() call failed for ${selector}, falling back to native options`
              );
            }
          }
          ...
          if (optionCount > 1) {
            ...
            const isSortingDropdown =
              firstOption === "alphabetically" ||
              firstOption === "price" ||
              firstOption === "context" ||
            ...
            if (isSortingDropdown) {
              console.log(
                `[Sidecar AI] Skipping sorting/grouping dropdown: ${selector}`
              );
              continue;
            }

            console.log(
              `[Sidecar AI] Found populated dropdown at ${selector} with ${optionCount} ${optionsSource}`
            );
            ...
            if (models.length > 0) {
              if (provider === "openrouter") {
                console.log(
                  `[Sidecar AI] OpenRouter dropdown has ${models.length} models, continuing to check globals for full list...`
                );
                // Don't return yet, continue to check globals which might have more
                // Keep models found so far, we'll use the best source later
              } else {
                console.log(
                  "[Sidecar AI] Successfully stole models from UI:",
                  models.length
                );
                return models;
              }
    ...
    if (provider === "openrouter") {
      ...
      if (typeof window !== "undefined") {
        ...
        if (
          window.model_list &&
          Array.isArray(window.model_list) &&
          window.model_list.length > 0
        ) {
        ...
        if (
          ...
          Array.isArray(window.openRouterModels) &&
          window.openRouterModels.length > 0
        ) {
        ...
        if (!modelsList && window.SillyTavern) {
          if (
            window.SillyTavern.model_list &&
            Array.isArray(window.SillyTavern.model_list) &&
            window.SillyTavern.model_list.length > 0
          ) {
            modelsList = window.SillyTavern.model_list;
            source = "SillyTavern.model_list";
          } else if (
            window.SillyTavern.openRouterModels &&
            Array.isArray(window.SillyTavern.openRouterModels) &&
            window.SillyTavern.openRouterModels.length > 0
          ) {
      ...
      if (modelsList && modelsList.length > 0) {
        console.log(
          `[Sidecar AI] Found ${modelsList.length} models from ${source}`
        );
        ...
        if (newModels.length > models.length) {
          console.log(
            `[Sidecar AI] ${source} has ${newModels.length} models (more than DOM's ${models.length}), using ${source}`
          );
          models = newModels;
          ...
          console.log(
            `[Sidecar AI] DOM has ${models.length} models, ${source} has ${newModels.length}, keeping DOM models`
          );
        }
        ...
        if (models.length > 0) {
          console.log(
            "[Sidecar AI] Loaded",
            models.length,
            "OpenRouter models successfully"
          );
          return models;
        }
      } else {
        console.warn(
          "[Sidecar AI] Could not find OpenRouter models in window.model_list or window.openRouterModels"
        );
        console.log(
          "[Sidecar AI] Available window properties:",
    ...
    if (
      provider === "openrouter" &&
      this.context &&
      this.context.chatCompletionSettings
    ) {
      const ccSettings = this.context.chatCompletionSettings;
      if (ccSettings.openrouter_providers) {
        console.log(
          "[Sidecar AI] Found openrouter_providers data type:",
          typeof ccSettings.openrouter_providers
        );
        ...
        try {
          ...
          const structure = JSON.stringify(
            ccSettings.openrouter_providers
          ).substring(0, 200);
        ...
        if (typeof providers === "object" && !Array.isArray(providers)) {
          for (const [providerName, providerData] of Object.entries(
            ...
            if (
              ...
              const pModels = Array.isArray(providerData.models)
                ? providerData.models
                : [];
              pModels.forEach((m) => {
              ...
              if (
                providerData[0] &&
                (providerData[0].id || providerData[0].name)
              ) {
                foundModels = true;
        ...
        if (foundModels && models.length > 0) {
          console.log(
            "[Sidecar AI] Loaded",
            models.length,
            "models from openrouter_providers"
          );
          return models;
        } else {
          console.log(
            "[Sidecar AI] openrouter_providers only contains provider names, not models. Continuing..."
          );
        }
      }
    }
    ...
    if (
      this.context &&
      this.context.mainApi &&
      Array.isArray(this.context.mainApi)
    ) {
      ...
      for (const profile of mainApi) {
        ...
        if (pName === provider.toLowerCase()) {
          ...
          const list =
            profile.models || profile.modelList || profile.availableModels;
    ...
    if (models.length === 0 && this.context && this.context.settings) {
      ...
      console.log(
        "[Sidecar AI] settings keys:",
        Object.keys(this.context.settings)
      );
      if (this.context.settings.connection_profiles) {
        ...
        const profiles = Object.values(
          this.context.settings.connection_profiles || {}
        );
        for (const profile of profiles) {
          if (
            profile &&
            (profile.api_provider === provider || profile.provider === provider)
          ) {
            console.log(
              "[Sidecar AI] Profile in settings keys:",
              Object.keys(profile)
            );
    ...
    if (
      models.length === 0 &&
      this.context &&
      this.context.connection_profiles
    ) {
    ...
    if (provider === "openrouter" && models.length === 0) {
      try {
        console.log(
          "[Sidecar AI] Strategy 4: Attempting to fetch OpenRouter models directly from API..."
        );
        ...
        if (response.ok) {
          ...
          if (data && data.data && Array.isArray(data.data)) {
            console.log(
              `[Sidecar AI] Fetched ${data.data.length} models from OpenRouter API`
            );
            data.data.forEach((m) => {
              models.push({
                value: m.id,
            ...
            console.log(
              `[Sidecar AI] Sorted ${models.length} OpenRouter models by provider and name`
            );
            return models;
          }
        }
  ...
  applyThemeColors() {
    ...
    const bgColor =
      ...
      computedStyle.getPropertyValue("--SmartThemeBodyColor") ||
      "#1e1e1e";
    const textColor =
      computedStyle.color ||
      computedStyle.getPropertyValue("--SmartThemeBodyColor") ||
      "#eee";
    const borderColor =
      computedStyle.getPropertyValue("--SmartThemeBorderColor") || "#555";
    ...
    const allElements = document.querySelectorAll(
      ".add_ons_modal_content, .add_ons_modal_content *"
    );
  ...
  async populateForm(addon) {
    ...
    if (addon.aiProvider === "openrouter") {
      ...
      setTimeout(() => {
        ...
        if (
          ...
          Array.isArray(addon.serviceProvider) &&
          addon.serviceProvider.length > 0
        ) {
          ...
          if (hasOptions) {
            ...
            console.log(
              "[Sidecar AI] Restored service provider selection:",
              addon.serviceProvider
            );
          } else {
            ...
            setTimeout(() => {
              $serviceProviderSelect.val(addon.serviceProvider);
              console.log(
                "[Sidecar AI] Restored service provider selection (delayed):",
                addon.serviceProvider
              );
            }, 200);
          }
    ...
    $("#add_ons_form_include_char_card").prop(
      "checked",
      ctx.includeCharCard !== false
    );
    $("#add_ons_form_include_user_card").prop(
      "checked",
      ctx.includeUserCard !== false
    );
    $("#add_ons_form_include_world_card").prop(
      "checked",
      ctx.includeWorldCard !== false
    );
  ...
  testRegexPatterns() {
    ...
    if (!testInput) {
      resultDiv
        .html(
          '<span style="color: #ffc107;">⚠️ Please enter a test message</span>'
        )
        .show();
      return;
    }
    ...
    if (!triggersText.trim()) {
      resultDiv
        .html(
          '<span style="color: #ffc107;">⚠️ Please enter at least one trigger pattern</span>'
        )
        .show();
      return;
    }
    ...
    if (triggerType === "regex") {
      ...
      triggers.forEach((pattern, index) => {
        try {
          ...
          if (matches) {
            ...
            results.push(
              `<div style="color: #28a745; margin: 4px 0;"><strong>✓ Pattern ${
                index + 1
              }: "${pattern}"</strong> - MATCHED</div>`
            );
          } else {
            results.push(
              `<div style="color: #6c757d; margin: 4px 0;">✗ Pattern ${
                index + 1
              }: "${pattern}" - No match</div>`
            );
          }
        } catch (e) {
          hasError = true;
          results.push(
            `<div style="color: #dc3545; margin: 4px 0;"><strong>✗ Pattern ${
              index + 1
            }: "${pattern}"</strong> - ERROR: ${e.message}</div>`
          );
        }
      });
    } else {
      ...
      triggers.forEach((keyword, index) => {
        ...
        if (matches) {
          hasMatch = true;
          results.push(
            `<div style="color: #28a745; margin: 4px 0;"><strong>✓ Keyword ${
              index + 1
            }: "${keyword}"</strong> - MATCHED</div>`
          );
        } else {
          results.push(
            `<div style="color: #6c757d; margin: 4px 0;">✗ Keyword ${
              index + 1
            }: "${keyword}" - No match</div>`
          );
        }
      });
    }
    ...
    if (hasMatch) {
      resultHTML +=
        '<div style="background: #d4edda; border: 1px solid #c3e6cb; color: #155724; padding: 8px; border-radius: 4px; margin-bottom: 8px;"><strong>✓ At least one pattern matched!</strong></div>';
    } else if (hasError) {
      resultHTML +=
        '<div style="background: #f8d7da; border: 1px solid #f5c6cb; color: #721c24; padding: 8px; border-radius: 4px; margin-bottom: 8px;"><strong>✗ Errors found in patterns</strong></div>';
    } else {
      resultHTML +=
        '<div style="background: #fff3cd; border: 1px solid #ffeaa7; color: #856404; padding: 8px; border-radius: 4px; margin-bottom: 8px;"><strong>⚠ No patterns matched</strong></div>';
    }

    resultHTML +=
      '<div style="font-size: 0.9em;">' + results.join("") + "</div>";
   ...
   */
  clearErrors() {
    $(".add_ons_field_error, .add_ons_field_success").removeClass(
      "add_ons_field_error add_ons_field_success"
    );
  }
  ...
  loadServiceProviders() {
    ...
    if ($stSelect.length > 0 && $stSelect.find("option").length > 0) {
      ...
      console.log(
        `[Sidecar AI] Loaded ${
          $serviceProviderSelect.find("option").length
        } service providers`
      );
    } else {
      console.log(
        "[Sidecar AI] OpenRouter providers dropdown not found, service providers not loaded"
      );
    }
  }
    ...
    try {
      ...
      const formData = {
        ...
        triggerConfig: {
          ...
          triggers: $("#add_ons_form_triggers")
            .val()
            .split("\n")
            .map((t) => t.trim())
            .filter((t) => t.length > 0),
  ...
  updateBulkActionBar() {
    ...
    if (this.selectedAddons.size > 0) {
      if (bulkBar.length === 0) {
        bulkBar = $(
          '<div id="add_ons_bulk_action_bar" class="add_ons_bulk_action_bar"></div>'
        );
        bulkBar.html(`
        ...
        bulkBar
          .find(".add_ons_bulk_action_info")
          .text(`${this.selectedAddons.size} item(s) selected`);
      }
      bulkBar.show();
    } else {
      if (bulkBar.length > 0) {
  ...
  bulkDisable() {
    ...
    const count = this.addonManager.bulkDisable(
  ...
  bulkDelete() {
    ...
    if (
      !confirm(
        `Are you sure you want to delete ${count} add-on(s)? This cannot be undone.`
      )
    ) {
      return;
    }

    const deleted = this.addonManager.bulkDelete(
      Array.from(this.selectedAddons)
    );
    if (deleted > 0) {
      this.clearSelection();
  ...
  bulkDuplicate() {
    ...
    const duplicated = this.addonManager.bulkDuplicate(
      Array.from(this.selectedAddons)
    );
  ...
  showHistoryViewer(addonId) {
    ...
    // Set modal title
    $("#add_ons_history_modal_title").text(
      `Result History: ${addon.name} (${results.length} results)`
    );
  ...
  renderHistoryList(results) {
    ...
    if (results.length === 0) {
      listContainer.html(
        '<div style="text-align: center; padding: 20px; color: rgba(255, 255, 255, 0.5);">No results found for this add-on.</div>'
      );
      return;
    }
    ...
    results.forEach((result, index) => {
      ...
      item.style.cssText =
        "padding: 12px; border: 1px solid var(--SmartThemeBorderColor, #555); border-radius: 5px; background: rgba(128, 128, 128, 0.05);";
      ...
      item.innerHTML = `
                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 8px;">
                    <div>
                        <strong style="color: #eee;">Result #${
                          results.length - index
                        }</strong>
                        ${
                          result.edited
                            ? '<span style="margin-left: 8px; padding: 2px 6px; background: rgba(255, 193, 7, 0.2); border: 1px solid rgba(255, 193, 7, 0.5); border-radius: 3px; color: #ffc107; font-size: 0.8em;">EDITED</span>'
                            : ""
                        }
      ...
      // Delete button
      $(item)
        .find('[data-action="delete-result"]')
        .on("click", () => {
  ...
  deleteHistoryResult(result, index) {
    ...
    if (
      message &&
      resultFormatter.deleteResultFromMetadata(message, result.addonId)
    ) {
      // Save chat
  ...
  handleExport() {
    try {
      ...
      const selectedIds =
        this.selectedAddons.size > 0 ? Array.from(this.selectedAddons) : null;
      ...
      // Export add-ons
      const exportData = this.addonManager.exportAddons(
        selectedIds,
        includeApiKeys
      );
      ...
      console.log(
        `[Sidecar AI] Exported ${exportData.addons.length} add-on(s)`
      );

      // Show success message
      if (selectedIds) {
        alert(
          `Successfully exported ${exportData.addons.length} selected add-on(s).`
        );
      } else {
  ...
  handleImport() {
    ...
    if (pasteOption) {
      ...
      const pastePrompt = prompt("Paste the JSON export data below:", "");
  ...
  processImport(importData) {
    try {
      ...
      if (
        !importData ||
        !importData.addons ||
        !Array.isArray(importData.addons)
      ) {
      ...
      const addonNames = importData.addons
        .slice(0, 5)
      ...
      if (mergeMode === "replace") {
        if (
          !confirm(
            `WARNING: This will replace ALL existing add-ons with the imported ones.\n\nAre you sure?`
          )
        ) {
          return;
        }
  ...
  async loadLocalTemplates() {
    ...
    listContainer.html(
      '<p style="opacity: 0.7;">Loading local templates...</p>'
    );
    ...
    try {
      ...
      for (const filename of templateFiles) {
        try {
          ...
          console.warn(
            `[Sidecar AI] Failed to load template: ${filename}`,
            error
          );
        }
      }
      ...
      if (listContainer.children().length === 0) {
        listContainer.html(
          '<p style="opacity: 0.7;">No templates found in templates folder.</p>'
        );
      }
    } catch (error) {
      console.error("[Sidecar AI] Error loading templates:", error);
      listContainer.html(
        '<p style="color: #ef4444;">Error loading templates. Check console for details.</p>'
      );
    }
  }
  ...
  async loadCommunityTemplates() {
    const listContainer = $("#add_ons_templates_list");
    listContainer.html(
      '<p style="opacity: 0.7;"><i class="fa-solid fa-spinner fa-spin"></i> Loading community templates from GitHub...</p>'
    );
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          listContainer.html(
            '<p style="opacity: 0.7;">No community templates available yet. Be the first to contribute!</p>'
          );
          return;
        }
      ...
      const jsonFiles = files.filter(
        (f) => f.name.endsWith(".json") && f.type === "file"
      );

      if (jsonFiles.length === 0) {
        listContainer.html(
          '<p style="opacity: 0.7;">No community templates found.</p>'
        );
        return;
      }
      ...
      for (const file of jsonFiles) {
        try {
          ...
          console.warn(
            `[Sidecar AI] Failed to load community template: ${file.name}`,
            error
          );
        }
      }
  ...
  renderTemplateCard(container, template, filename, isCommunity = false) {
    ...
    const card = $("<div></div>")
      ...
      .hover(
        function () {
          $(this).css("border-color", "var(--SmartThemeEmColor)");
        },
        function () {
    ...
    if (isCommunity) {
      const communityBadge = $("<span></span>")
        .css({
          padding: "2px 6px",
          "border-radius": "3px",
  ...
  loadConnectionProfiles() {
    ...
    try {
      ...
      if (profiles.length === 0) {
        select.append(
          '<option value="">No API connections configured</option>'
        );
        return;
      }
...
Return ONLY the JSON object, properly formatted.`;
      ...
      if (
        ...
        model =
          profile.models[0].id || profile.models[0].name || profile.models[0];
      }
      ...
      if (provider?.toLowerCase() === "openrouter") {
        ...
        if (realModels && realModels.length > 0) {
          ...
          if (
            ...
            for (const preferred of preferredModels) {
              const found = realModels.find((m) => {
                ...
                return (
                  val === preferred || val.includes(preferred.split("/")[1])
                );
              });
              if (found) {
            ...
            if (!foundModel && realModels.length > 0) {
              foundModel =
                realModels[0].value || realModels[0].id || realModels[0];
            }

            if (foundModel) {
              console.log(
                `[Sidecar AI] OpenRouter: Replaced placeholder model "${model}" with real model "${foundModel}"`
              );
              model = foundModel;
            }
          }
          ...
          console.warn(
            "[Sidecar AI] Could not load OpenRouter models, using default"
          );
      ...
      const codeFenceMatch = jsonStr.match(
        /^```(?:json)?\s*\n([\s\S]*)\n```\s*$/
      );
      if (codeFenceMatch) {
  ...
  exportGeneratedTemplate() {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `${this.generatedTemplate.name
        .replace(/[^a-z0-9]/gi, "-")
  ...
  addGeneratedTemplate() {
    ...
    try {
      ...
      if (
        !confirm(
          `Add "${this.generatedTemplate.name}" as a new sidecar?\n\nYou can edit it later to customize settings.`
        )
      ) {
        return;
      }
      ...
      console.log(
        "[Sidecar AI] Added generated template as sidecar:",
        addon.name
      );
      alert(
        `✓ Sidecar created: ${addon.name}\n\nDon't forget to add your API key!`
      );
  ...
  async testTemplate() {
    ...
    try {
        ...
        messages: [
          {
            role: "user",
            content:
              "You walk into a cozy coffee shop on a rainy afternoon. The bell above the door chimes as you enter, and you see a familiar face at a corner table.",
          },
          {
            role: "assistant",
            content:
              '*I look up from my book, a warm smile spreading across my face as I recognize you. I set the book down and gesture to the empty chair across from me.*\n\n"Well, well, look who decided to brave the weather. Come, sit. I was just thinking about you."\n\n*The aroma of freshly brewed coffee fills the air, and soft jazz plays in the background. Raindrops trace patterns on the window beside us.*',
          },
          {
            role: "user",
            content:
              'I slide into the chair, shaking off my wet coat. "You always know the best spots. What are you reading?"',
          },
          {
            role: "assistant",
            content:
              '*I pick up the book, showing you the cover - a worn copy of "The Night Circus"*\n\n"Just re-reading an old favorite. There\'s something about magical realism that feels perfect for days like this, don\'t you think?"\n\n*I lean forward slightly, my eyes sparkling with curiosity*\n\n"But enough about books. Tell me - what brings you here today? You seem... contemplative."',
          },
        ],
        character: {
          name: "Alex",
          description:
            "A thoughtful, warm-hearted person who loves books, coffee, and deep conversations. They have a gentle sense of humor and are always genuinely interested in others.",
        },
      };
      ...
      let model =
        ...
        this.generatedTemplate.aiModel ||
        "gpt-4o-mini";
      const presetName = $("#ai_maker_preset").val() || undefined;
      ...
      const messages = [
        {
          role: "system",
          content:
            "You are a task executor. Your ONLY job is to follow the instruction block provided by the user. DO NOT continue stories, generate dialogue, or roleplay. ONLY execute the specific task requested in the instruction block.",
        },
        {
          role: "user",
          content: fullPrompt,
      ...
      console.log("[Sidecar AI] Testing template with scenario:", {
        provider,
        model,
        chatCompletionSource,
      });
      ...
      // Scroll to result
      $("#ai_maker_test_result")[0].scrollIntoView({
        behavior: "smooth",
        block: "nearest",
      });
  ...
  async publishTemplate() {
    ...
    const templateName = this.generatedTemplate.name
      .replace(/[^a-z0-9]/gi, "-")
      .toLowerCase();
    ...
    try {
      ...
      alert(
        "Error preparing template for GitHub: " +
          error.message +
