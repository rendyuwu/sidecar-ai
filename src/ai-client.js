...
export class AIClient {
  ...
  async sendToAI(addon, prompt, retryCount = 0) {
    try {
      ...
      if (this.context && this.context.ChatCompletionService) {
        if (retryCount === 0) {
          console.log(
            `[Sidecar AI] Using SillyTavern ChatCompletionService for ${provider} (${model})`
          );
        } else {
          console.log(
            `[Sidecar AI] Retry attempt ${retryCount}/${this.retryConfig.maxRetries} for ${addon.name}`
          );
        }
        ...
        if (Array.isArray(prompt)) {
          ...
          // Add prefill if specified in addon configuration
          if (addon.prefill) {
            messages.push({
              role: "assistant",
              content: addon.prefill,
            });
          }
        }
        ...
        const response =
          await this.context.ChatCompletionService.processRequest(
            requestOptions,
            {
      ...
      console.warn(
        "[Sidecar AI] ChatCompletionService not available, using fallback"
      );
      return await this.sendDirectAPIFallback(
        addon,
        prompt,
        provider,
        model,
      ...
      if (this.shouldRetry(error, retryCount)) {
        ...
        console.log(
          `[Sidecar AI] Retrying ${addon.name} after ${delay}ms (attempt ${
            retryCount + 1
      ...
      console.error(
        `[Sidecar AI] Error sending to AI (${addon.name}) after ${retryCount} retries:`,
        error
      );
      throw error;
    }
  ...
  async sendBatchToAI(addons, prompts) {
    ...
    const allSame = addons.every(
      (addon) => addon.aiProvider === provider && addon.aiModel === model
    );
    ...
    try {
      const apiKey =
        addons[0].apiKey || (await this.getProviderApiKey(provider));
      ...
      if (this.context && this.context.ChatCompletionService) {
        ...
        // Add prefill if specified in addon configuration (for batch requests)
        if (addons[0].prefill) {
          messages.push({
            role: "assistant",
            content: addons[0].prefill,
          });
        }
  ...
  async sendDirectAPIFallback(addon, prompt, provider, model, apiUrl = null) {
    ...
    if (provider === "openrouter") {
      ...
      headers["HTTP-Referer"] =
        window.location.origin || "https://github.com/skirianov/sidecar-ai";
    ...
    if (parts.length !== count) {
      ...
      if (markers.length === count) {
        ...
        for (let i = 0; i < markers.length; i++) {
          ...
          const end =
            i < markers.length - 1 ? markers[i + 1].index : response.length;
  ...
  async testConnection(
    ...
    try {
      ...
      const corsBlockingProviders = [
        "deepseek",
        "anthropic",
        "google",
        "cohere",
      ];
      const isCorsBlocking = corsBlockingProviders.includes(
      ...
      if (
        this.context &&
        this.context.ChatCompletionService &&
        (isUsingSTKey || isCorsBlocking)
      ) {
        console.log(
          `[Sidecar AI] Testing connection via ChatCompletionService: ${provider} (${model})${
            isUsingSTKey ? " (using ST saved key)" : ""
          }`
        );
        ...
        try {
          ...
          try {
            ...
            console.warn(
              "[Sidecar AI] ChatCompletionService test failed:",
              chatServiceError
            );
            console.warn("[Sidecar AI] Error type:", typeof chatServiceError);
            console.warn(
              "[Sidecar AI] Error keys:",
              chatServiceError ? Object.keys(chatServiceError) : "null"
            );
            ...
            if (chatServiceError && typeof chatServiceError === "object") {
              ...
              if (chatServiceError.response) {
                console.warn(
                  "[Sidecar AI] Error has response property:",
                  chatServiceError.response
                );
              }
              // Check if there's a cause property (from newer Error objects)
              if (chatServiceError.cause) {
                console.warn(
                  "[Sidecar AI] Error has cause property:",
                  chatServiceError.cause
                );
              }
              // Log all enumerable and non-enumerable properties
              console.warn(
                "[Sidecar AI] Error object properties:",
                Object.getOwnPropertyNames(chatServiceError)
              );
              for (const key in chatServiceError) {
                console.warn(
                  `[Sidecar AI] Error.${key}:`,
                  chatServiceError[key]
                );
              }
            }
          ...
          console.warn(
            "[Sidecar AI] ChatCompletionService error handling:",
            chatServiceError
          );
          ...
          if (chatServiceError && typeof chatServiceError === "object") {
            ...
            if (
              ...
              typeof chatServiceError.error === "object"
            ) {
              errorMsg =
            ...
            // Check for string error
              ...
              chatServiceError.error &&
              typeof chatServiceError.error === "string"
            ) {
            ...
            // Try to find any string property that might be an error message
              ...
              const stringProps = Object.entries(chatServiceError)
                .filter(
                  ([k, v]) =>
                    typeof v === "string" && v.length > 0 && k !== "error"
                )
                .map(([k, v]) => `${k}: ${v}`);
          ...
          if (
            ...
            errorMsg =
              `Bad Request (400) - The API rejected the request.\n\n` +
              `Common causes:\n` +
      ...
      if (apiKey && !isCorsBlocking) {
        console.log(
          `[Sidecar AI] Testing connection directly: ${provider} (${model})`
        );
      ...
      if (!apiKey && !this.context?.ChatCompletionService) {
        throw new Error(
          "No API key provided and ChatCompletionService not available"
        );
      }

      throw new Error(
        "Unable to test connection - please configure provider in SillyTavern API Connection settings"
      );
    } catch (error) {
      ...
      if (errorMessage.includes("404") || errorMessage.includes("Not Found")) {
        return {
          success: false,
          message: "Invalid model or endpoint - verify model name and API URL",
        };
      }
  ...
  async testConnectionDirect(provider, model, apiKey, apiUrl = null) {
    ...
    try {
      ...
      console.log(
        `[Sidecar AI] Response status: ${response.status} ${response.statusText}`
      );

      // Handle status 0 (network/CORS error)
      if (response.status === 0) {
        throw new Error(
          "Network error: Request blocked (likely CORS issue). Deepseek API may require server-side requests or a proxy."
        );
      }
      ...
      if (!content && !data.choices && !data.content) {
        ...
        if (data.id || data.model || data.usage) {
          return {
            success: true,
            message: "Connection successful (empty response)",
          };
        }
        throw new Error(
          "Invalid response format - no content or choices found"
        );
      }
      ...
      if (error.name === "TypeError" && error.message.includes("fetch")) {
        ...
        throw new Error(
          `Network error: ${error.message}. Check if endpoint is correct and CORS is enabled.`
        );
      }
  ...
  getRequestHeaders() {
    ...
    if (!headers && typeof window !== "undefined") {
      if (
        ...
        try {
          ...
          console.warn(
            "[Sidecar AI] Error calling window.getRequestHeaders:",
            e
          );
        }
      }
      ...
      if (
        ...
        typeof window.SillyTavern.getRequestHeaders === "function"
      ) {
        try {
          headers = window.SillyTavern.getRequestHeaders();
          console.log(
            "[Sidecar AI] Got headers from window.SillyTavern.getRequestHeaders"
          );
        } catch (e) {
          console.warn(
            "[Sidecar AI] Error calling window.SillyTavern.getRequestHeaders:",
            e
          );
        }
      }
    ...
    if (
      ...
      typeof this.context.getRequestHeaders === "function"
    ) {
      try {
        ...
        console.warn(
          "[Sidecar AI] Error calling context.getRequestHeaders:",
          e
        );
      }
    }
    ...
    if (!headers) {
      console.warn(
        "[Sidecar AI] getRequestHeaders not found, using basic headers (may cause 403)"
      );
      headers = {
        "Content-Type": "application/json",
      };
  ...
  async findSecret(key, id = null) {
    ...
    if (
      ...
      window.findSecret &&
      typeof window.findSecret === "function"
    ) {
      try {
        console.log(
          `[Sidecar AI] Using window.findSecret for ${key}${
            id ? ` (id: ${id})` : ""
          }`
        );
        ...
        if (result) {
          console.log(
            "[Sidecar AI] Successfully got secret via window.findSecret"
          );
          return result;
        }
      } catch (e) {
        console.warn(
          "[Sidecar AI] window.findSecret failed, trying direct API call:",
          e
        );
      }
    }
    ...
    try {
      ...
      console.log(
        `[Sidecar AI] Fetching secret via API: ${key}${
          id ? ` (id: ${id})` : ""
        }`
      );
      ...
      if (!response.ok) {
        ...
        console.warn(
          `[Sidecar AI] API returned ${response.status} for secret ${key}:`,
  ...
  async hasProviderApiKey(provider) {
    ...
    console.log(
      `[Sidecar AI] hasProviderApiKey: Checking for ${provider} (${providerLower})`
    );
    ...
    if (
      ...
      this.context.extensionSettings.connectionManager
    ) {
      const profiles =
        this.context.extensionSettings.connectionManager.profiles || [];
      console.log(
        `[Sidecar AI] Checking ${profiles.length} connection manager profiles`
      );

      for (const profile of profiles) {
        const profileApi = profile?.api?.toLowerCase();
        console.log(
          `[Sidecar AI] Profile: api=${profileApi}, hasSecretId=${!!profile?.[
            "secret-id"
          ]}`
        );
        ...
        if (profile && profileApi === providerLower && profile["secret-id"]) {
          console.log(
            `[Sidecar AI] Found connection profile with secret-id for ${provider}`
          );
          return true;
        }
      }
    ...
    if (secretState && secretKey) {
      ...
      if (secrets) {
        ...
        if (Array.isArray(secrets)) {
          if (secrets.length > 0) {
            console.log(
              `[Sidecar AI] Found secret_state entry for ${provider} (${secrets.length} secrets)`
            );
            return true;
          }
        } else {
          ...
          console.log(
            `[Sidecar AI] Found secret_state entry for ${provider} (non-array, truthy)`
          );
          return true;
        }
      } else {
      ...
      if (!secretKey) {
        console.log(
          `[Sidecar AI] No secret key mapping for provider: ${provider}`
        );
      }
    }
  ...
  async getProviderApiKey(provider) {
    ...
    console.log(
      `[Sidecar AI] getProviderApiKey: Looking for API key for provider: ${provider}`
    );
    ...
    if (
      ...
      const profiles =
        this.context.extensionSettings.connectionManager.profiles || [];
      console.log(
        `[Sidecar AI] Found ${profiles.length} connection manager profiles`
      );

      for (const profile of profiles) {
        console.log(`[Sidecar AI] Checking profile:`, {
          api: profile?.api,
          name: profile?.name,
          hasSecretId: !!profile?.["secret-id"],
        });
        ...
        if (profile && profileApi === providerLower && profile["secret-id"]) {
          ...
          console.log(
            `[Sidecar AI] Found matching profile! secretId: ${secretId}, secretKey: ${secretKey}`
          );

          // secret_state doesn't have the actual value, fetch it via API
          if (secretKey) {
            console.log(
              `[Sidecar AI] Method 1: Fetching secret via API: ${secretKey}, id: ${secretId}`
            );
            const apiKey = await this.findSecret(secretKey, secretId);
            if (apiKey) {
              console.log(
                `[Sidecar AI] Successfully fetched API key via API (Method 1)`
              );
              return apiKey;
            } else {
              console.log(
                `[Sidecar AI] API returned null for ${secretKey} with id ${secretId}`
              );
            }
          } else {
            console.log(
              `[Sidecar AI] No secretKey mapped for provider: ${provider}`
            );
          }
        }
      }
    ...
    const secretKey = this.getSecretKeyForProvider(provider);
    console.log(
      `[Sidecar AI] Method 2: secretKey for ${provider}: ${secretKey}`
    );
    ...
    if (secretKey) {
      ...
      if (secretState && secretState[secretKey]) {
        const secrets = secretState[secretKey];
        console.log(
          `[Sidecar AI] Found secrets in secret_state for ${secretKey}:`,
          Array.isArray(secrets) ? secrets.length : "not an array"
        );
        ...
        if (Array.isArray(secrets) && secrets.length > 0) {
          ...
          if (secretId) {
            console.log(
              `[Sidecar AI] Method 2: Fetching secret via API with ID: ${secretId}`
            );
            const apiKey = await this.findSecret(secretKey, secretId);
            if (apiKey) {
              console.log(
                `[Sidecar AI] Successfully fetched API key via API (Method 2)`
              );
              return apiKey;
            }
          }
      ...
      console.log(
        `[Sidecar AI] Method 3: Attempting API fetch for ${secretKey} (no ID)`
      );
      const apiKey = await this.findSecret(secretKey);
      if (apiKey) {
        console.log(
          `[Sidecar AI] Successfully fetched API key via API (Method 3)`
        );
        return apiKey;
      }
    }
    ...
    if (this.context && this.context.connection_profiles) {
      ...
      for (const profile of profiles) {
        if (
          profile &&
          (profile.api_provider === provider || profile.api === provider)
        ) {
          if (profile.api_key) {
    ...
    if (
      ...
      this.context.settings.api_keys
    ) {
    ...
    if (typeof window !== "undefined") {
      if (window.SillyTavern && window.SillyTavern.api_keys) {
        ...
        if (key) {
          console.log(
            `[Sidecar AI] Found API key in window.SillyTavern.api_keys`
          );
          return key;
        }
      }
